#! python3
# This is an old version and needs updating, it's definitey not functional at the moment
# xmlwriter.py a program to convert our code to musicxml files
# formatted for MuseScore
# TODO -> create a more generic xmlwriter without all the MuseScore specific stuff
...
def xmlwrite():
    ...
    """if config.scalepattern == "yes" and config.grid != "yes":
        ...
        noteAttributess = config.scale_pattern_attributelist"""
    ...
    for exercise_num in exercise_list: #aka for ex_number in exercise_list.keys() 
        ...
        #KEY/CLEF/TIME SIGNATURE #TODO MAKE CLEF VISIBILITY OPTION
        if int(exercise_num) == 1: #CLEF AND TIME SIG FOR FIRST MEASURE    
            ...
            #SubElementWithText(clef, "line", config.clef_line) #TODO clef is being doubled for some reason...
        ...
        #REHEARSAL MARKS AKA MEASURE NUMBERS
        ...
        if config.grid != "yes" and config.scalepattern != "yes":
            for ex_note in exercise_list[exercise_num].keys(): #
                ...
                if 'flat' in ex_note:
                    SubElementWithText(pitch, "alter", "-1")
                noteAttributess = exercise_list[exercise_num][ex_note]
                SubElementWithText(pitch, "octave", noteAttributess[0])
                SubElementWithText(note, "duration", noteAttributess[1])
                SubElementWithText(note, "voice", noteAttributess[2])
                SubElementWithText(note, "type", noteAttributess[3])
                if ex_note not in scales.allscales[config.scaletype][config.key_sig]: #what is this for?
                    if 'sharp' in ex_note:
                        SubElementWithText(note, "accidental", "sharp")
                    if 'flat' in ex_note:
                        SubElementWithText(note, "accidental", "flat")
                SubElementWithText(note, "stem", noteAttributess[4])
                if clef == "Grand Staff": 
                    SubElementWithText(note, "staff", noteAttributess[5])
            ...
            if clef == "Grand Staff":
                ...
                for ex_note in exercise_list_staff2[exercise_num].keys():
                    ...
                    if 'flat' in ex_note:
                        SubElementWithText(pitch, "alter", "-1")
                    noteAttributess = exercise_list_staff2[exercise_num][ex_note]
                    SubElementWithText(pitch, "octave", noteAttributess[0])
                    SubElementWithText(note, "duration", noteAttributess[1])
                    SubElementWithText(note, "voice", noteAttributess[2])
                    SubElementWithText(note, "type", noteAttributess[3])
        ...
        #GRID STAFF -->
        '''
        In this case, grid means a simplified grid where the pitches don't change
        For an mpm with a grid, we would use the regular xmlwriter functions
        ...
        if config.scalepattern == "yes":
            ...
            for ex in exercise_list:
                for ex_note in exercise_list[ex]:
                    ...
                    SubElementWithText(pitch, "octave", noteAttributess["octave"])
                    SubElementWithText(note, "duration", noteAttributess["duration"])
                    SubElementWithText(note, "voice", "1")
                    SubElementWithText(note, "type", noteAttributess["type"])
